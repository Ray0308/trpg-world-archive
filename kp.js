...
(function () {
  ...
  const EXTERNAL_LINK_DEFS = [
    { key: 'cocofolia', name: 'ココフォリア', hint: '部屋URL（例: https://ccfolia.com/rooms/...）' },
    { key: 'discord', name: 'Discord', hint: '招待URLまたはチャンネルURL' },
    { key: 'iachara', name: 'いあきゃら', hint: 'トップまたは特定ページURL' }
  ];

  async function renderExternalLinksSection() {
    const section = document.getElementById('kpExternalLinks');
    if (!section) return;

    section.innerHTML = `
      <h2 class="kp-section-title" id="kpExternalLinksTitle">外部リンク（PLサイト）</h2>
      <p class="kp-section-lead">ココフォリアの部屋URLなどは DBPJ の SETTINGS シートで変更できます。</p>
      <p class="kp-modal-status">読み込み中...</p>
    `;

    let links = { ...window.AppLinks };
    let settingsUrl = '';

    try {
      const base = window.AppConfig?.api?.baseUrl;
      if (base && window.AppsScriptProvider?.fetchLinks) {
        const data = await window.AppsScriptProvider.fetchLinks(base, { kp: true });
        settingsUrl = String(data._settings_sheet_url || '').trim();
        window.applyAppLinkOverrides?.(data);
        links = { ...window.AppLinks };
      }
    } catch (err) {
      console.warn('[KP links]', err);
    }

    const rows = EXTERNAL_LINK_DEFS.map(item => {
      const url = String(links[item.key] || '').trim();
      const display = url && url !== '#' ? url : '（未設定 — SETTINGS または links.js）';
      return `
        <li class="kp-link-row">
          <div class="kp-link-row-main">
            <p class="kp-link-row-name">${escapeHtml(item.name)}</p>
            <p class="kp-link-row-url">${escapeHtml(display)}</p>
            <p class="kp-link-row-hint">${escapeHtml(item.hint)}</p>
          </div>
          ${url && url !== '#'
            ? `<a href="${escapeAttr(url)}" class="kp-card-btn kp-card-btn--link" target="_blank" rel="noopener noreferrer">開く${EXTERNAL_ICON}</a>`
            : ''}
        </li>
      `;
    }).join('');
    ...
    section.innerHTML = `
      <h2 class="kp-section-title" id="kpExternalLinksTitle">外部リンク（PLサイト）</h2>
      <p class="kp-section-lead">ココフォリアの部屋URLなどは DBPJ の SETTINGS シートで変更できます。保存後、PLサイトを再読み込みすると反映されます。</p>
      <article class="kp-card kp-card--drive">
        <span class="kp-card-icon">🔗</span>
        <h3 class="kp-card-name">SETTINGS シート</h3>
        <p class="kp-card-desc">
          <code>cocofolia</code> の value に部屋URLを貼り付けてください。
          空欄の項目は <code>links.js</code> の既定値が使われます。
        </p>
        <div class="kp-card-actions">
          ${settingsUrl
            ? `<a href="${escapeAttr(settingsUrl)}" class="kp-card-btn" target="_blank" rel="noopener noreferrer">SETTINGS を開く${EXTERNAL_ICON}</a>`
            : '<span class="kp-card-btn kp-card-btn--disabled" aria-disabled="true">API 未対応（NPCPJ を再デプロイ）</span>'}
        </div>
        <ul class="kp-link-list">${rows}</ul>
      </article>
    `;
  }
  ...
  async function boot() {
    ...
    await renderExternalLinksSection();
    bindModal();
  }
